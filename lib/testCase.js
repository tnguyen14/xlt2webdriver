...
module.exports = function (testcase) {
	...
	var setup = indent.repeat(2) + 'before(function () {' + newLine
		+ indent.repeat(3) + 'return webdriver.init().then(function () {' + newLine
		+ indent.repeat(4) + 'return webdriver.url(data.shopURL)});' + newLine
		+ indent.repeat(2) + '});' + newLine;
	var teardown = indent.repeat(2) + 'after(function (done){' + newLine
		+ indent.repeat(3) + 'webdriver.localStorage(\'DELETE\');'
