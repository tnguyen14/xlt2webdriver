...
module.exports = function(command, padding) {
	...
	if (c.type === 'action') {
		...
		// if both value and target are set
		if (command.attributes.target && command.attributes.value) {
			params.push(parseParam(command.attributes.target), parseParam(command.attributes.value));
		} else {
			params.push(parseParam(command.attributes.value || command.attributes.target));
		}
		if (c.reverse) {
	...
	return indent.repeat(padding) + (comment ? '// ' : '') + comment + newLine
		+ indent.repeat(padding) + 'describe(\'' + command.attributes.name + '\', function () {' + newLine
		+ indent.repeat(padding + 1) + 'it("' + command.attributes.name + '", function () {' + newLine
		+ indent.repeat(padding + 2) + 'return webdriver.' + c.name
		+ '(' + params.join(', ') + ')'
		+ '.then(function (res) {' + newLine
		+ indent.repeat(padding + 3) + 'assert.' + assert.command
		+ '(' + assert.values.join(', ') + ');' + newLine
		+ indent.repeat(padding + 2) + '});' + newLine
