...
module.exports = function(command, padding) {
	...
	if (c.type === 'action') {
		...
		// http://webdriver.io/api/action/selectorExecute.html
		if (c.name === 'selectorExecute') {
			params[1] = 'function (items) {' + newLine
				+ indent.repeat(padding + 2) + 'return items.length' + newLine
				+ indent.repeat(padding + 1) + '}';
			params[2] = 'undefined';
		}
